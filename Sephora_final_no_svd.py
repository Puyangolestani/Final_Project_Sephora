...
def load_model_and_objs():
    model = tf.keras.models.load_model("model_no_svd.keras")  # Note: Updated model name
    ...
    if not review_title.strip() and not review_text.strip():
        ...
        with st.spinner("Predicting..."):
            ...
            input_df = pd.DataFrame([{
                ...
                "skin_tone": skin_tone,
                "skin_type": skin_type,
                "is_recommended": "unknown"  # Dummy value
            }])
            ...
            with open("prediction_log.csv", "a", encoding="utf-8") as f:
                f.write(f"{datetime.datetime.now()},{skin_tone},{skin_type},{recommended},\"{full_review}\",{pred_rounded:.1f}\n")
...
if batch_file is not None:
    try:
        ...
        required_cols = {'review_title', 'review_text', 'skin_tone', 'skin_type'}
        if not required_cols.issubset(set(df_batch.columns)):
            ...
            df_input = df_batch[['full_review', 'skin_tone', 'skin_type', 'is_recommended']]
...
try:
    ...
    y_pred = joblib.load("y_pred_no_svd.pkl")  # Updated to reflect new predictions
