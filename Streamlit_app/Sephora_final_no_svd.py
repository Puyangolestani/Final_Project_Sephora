...
import datetime
# Add after all imports (e.g., after datetime)
...
def get_theme_color_by_rating(rating):
    if rating >= 4.5:
        return "#ffe6f0"  # light pink (excellent)
    elif rating >= 3.5:
        return "#fff8dc"  # light cream (good)
    elif rating >= 2.5:
        return "#f5f5f5"  # soft gray (neutral)
    elif rating >= 1.5:
        return "#ffe4e1"  # light rose (poor)
    else:
        return "#ffcccc"  # soft red (bad)
...
if submitted:
    if not review_title.strip() and not review_text.strip():
        ...
        with st.spinner("Predicting..."):
            ...
            bg_color = get_theme_color_by_rating(pred_rounded)
